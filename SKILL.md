...
- **v4.0.2** — Public-release hygiene: removed the executed `docs/global-expansion-plan.md` design record (its surviving rules live in this file's Reference Files section, the README Maintenance section, and `references/authorities.md`), completed the README project-structure tree, synced `package.json`'s version. No behavior change.

- **v4.0.1** — Cover generation notice trimmed to its first sentence ("AI-GENERATED DRAFT — produced by the dpia-generator skill."); the not-legal-advice framing already lives in the README disclaimer and the Step 6 delivery summary, and repeating it on the cover was surplus. Footer and metadata unchanged.
- **v4.0** — Generation transparency and the conditional consultation pathway. Every output now self-identifies as an AI-generated draft of this skill — cover notice, footer ("AI-generated draft (dpia-generator) — for attorney review", replacing "Prepared by Counsel"), file metadata, and the revision-history author — builder-owned with no manifest knob; the voice rules now forbid attributing the draft to a person, and the sign-off block is where a human adopts it. Separately, register rows may score the **post-mitigation residual** (`mitigatedLikelihood`/`mitigatedSeverity`, gated like every rating), making the consultation conclusion tri-state: `true`, `false`, or `"conditional"` — required *only if* the Section 5 mitigations are not implemented before processing, per Art. 36(1)'s "absence of measures" language. The register gains a Post-mitigation column, the matrix a `mitigated` stage, the engagement table a conditional label, and the gate forces the honest conditional answer in both directions (declaring unconditional "required" when the scores show a pathway is exit 3, exactly as understating is). Major bump: every output's footer and cover change. Suite to thirty-two cases.
