...
These are not single analogs but standing collections of published assessments and decisions, useful in Step 1 when nothing above matches the processing. Surfaced by web search 2026-08-10 (EDPB registers and GDPRhub: 2026-08-29); verify on use.

- **EDPB Register of final one-stop-shop decisions (Art. 60)** — https://www.edpb.europa.eu/our-work-tools/consistency-findings/register-for-article-60-final-decisions_en — the primary index of national DPA enforcement decisions under the cooperation mechanism, searchable by topic; the richest hunting ground for "what did a DPA actually decide about this processing type". The register's summaries are Secretariat-authored and non-authoritative — cite the underlying national decision, fetched, never the summary.
- **EDPB Art. 65 binding decisions** — https://www.edpb.europa.eu/our-work-tools/consistency-findings/binding-decisions_en — the Board's binding dispute resolutions; the strongest signal of the harmonised EU position on contested questions (behavioural-ads lawful basis, children's data, transfer suspensions). Few but heavyweight; check here whenever the processing sits on a known DPA fault line.
- **EDPB BCR approvals (Art. 64 opinions) and adequacy materials** — edpb.europa.eu; the Commission's adequacy-decisions page for the live adequacy list. These ground the transfer analysis; consumption rules (live-check at DPIA time, capture the approving SA and date) are in `references/authorities.md`.
- **GDPRhub (gdprhub.eu)** — wiki-sourced index of DPA and court decisions, often the fastest *finder* for national decisions the EDPB registers summarise thinly. Find-only, never cite: the citation is to the primary decision, fetched; wiki content never upgrades a source tag.
